...
async function main() {
  ...
  const deployments = {
    ...
    venueScan: {
      prediction: {
        ...
        evidence:
          "No verified native prediction venue with a permissionless market buy ABI was found in Somnia docs or Shannon explorer searches; using a deployed real market venue."
      },
      trading: {
        option: "NATIVE",
        chosen: "Somnia Exchange Algebra SwapRouter",
        evidence:
          "Verified SwapRouter, AlgebraFactory, pool deployer, WNative, USDT.g token, and WNative/USDT.g pool all returned code on Shannon; poolByPair returned a liquid pool."
      },
      lending: {
        option: "C",
        chosen: "Onward MiniLendingPool",
        evidence:
          "Native lending docs/explorer searches did not yield a verified permissionless supply ABI safe for Vault integration; using a deployed real lending pool."
      }
    }
  };
  ...
  const marketReceipt = await send(
    ...
    predictionMarket.createMarket("Onward v2 live-threshold challenge market")
  );
  ...
  writeJson("deployments-v2.json", deployments);
  console.log("wrote deployments-v2.json");
}
